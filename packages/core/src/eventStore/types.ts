...
> = (
  ...
  options?: {
    ...
    /**
     * The snapshot that seeded `prevAggregate` (from a preceding
     * `getAggregate`), used by the snapshot policy to decide whether to save
     * a new snapshot. Omitted means "unknown" — only history-free policies
     * fire (see {@link SeedSnapshot}). Only relevant when `prevAggregate` is
     * supplied or the event is version 1 (otherwise no snapshot is saved).
     */
...
> = (
  ...
  options?: {
    prevAggregate?: $AGGREGATE;
    /** See {@link EventPusher}'s `seedSnapshot`. */
    seedSnapshot?: SeedSnapshot<$AGGREGATE>;
    validate?: ValidateEventDetail;
  },
...
> = (
  ...
  /**
   * The snapshot that seeded this read (`{ status: 'present' }`) or a
   * confirmation that none exists (`{ status: 'absent' }`). Thread this back
   * into the next `pushEvent` / `groupEvent` (an {@link AggregateHandle} does
   * this automatically) so the snapshot policy can evaluate spacing.
   */
  seedSnapshot: SeedSnapshot<AGGREGATE>;
}>;
