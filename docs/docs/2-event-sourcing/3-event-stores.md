...
- <code>openAggregateFrom <i>((aggregate: Aggregate) => AggregateHandle)</i></code>: Wraps an aggregate you already hold, **without reading storage** (and synchronously, since it performs no I/O) — <code>aggregateId</code> and the pinned version are taken from the aggregate itself. This is the **least common** method, for unusual situations only: replay / bulk-import flows or any path where you already have a current aggregate in hand (e.g. from <code>getAggregateAndEvents</code>, a projection, or a simulation). **Avoid it inside a [command](./5-pushing-events.md#defining-a-command)** — there the aggregate should be read inside the handler (via <code>openAggregate</code> / <code>openExistingAggregate</code>) so each [retry](./5-pushing-events.md#race-conditions--retries) re-reads fresh state; a pre-loaded aggregate would replay a stale version on retry.
