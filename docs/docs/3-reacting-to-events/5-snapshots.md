...
`getEventsAndAggregate` / `getExistingEventsAndAggregate` use snapshots too, but they let the caller control _which events_ end up in the returned `events` array. Three mutually-exclusive options shape the read:
